...
function moduleScoreOf(text) {
  const progress = getSection(text, MODULE_SECTION_HEADINGS.progress)
  const match = progress === null ? null : /完成度\s*[:：]\s*(\d{1,3})/.exec(progress)
  const declared = match === null ? 0 : clampPercent(Number(match[1]))
  if (declared > 0) return declared
  const detail = countItems(getSection(text, MODULE_SECTION_HEADINGS.detail))
  const points = countItems(getSection(text, MODULE_SECTION_HEADINGS.points))
...
function initScore(sections, modules) {
...
function moduleEntries(puzzleDir, modules) {
  return modules.map((name) => {
    ...
    const sections = text === null ? { points: null, related: null, detail: null } : {
      points: getSection(text, MODULE_SECTION_HEADINGS.points),
      related: getSection(text, MODULE_SECTION_HEADINGS.related),
      detail: getSection(text, MODULE_SECTION_HEADINGS.detail),
    }
    return {
      name,
      file,
...
export function readState(projectRoot, projectName) {
  ...
  const pieces = [
    pieceOf('init', 'init', '初始化', initScore(sections, modules)),
...
export function readModuleDetail(projectRoot, projectName, moduleName) {
  ...
  if (!isFile(file)) {
    return { ok: true, name: slug, exists: false, file, progress: null, points: null, related: null, detail: null }
  }
  const text = readText(file)
  if (text === null) return { ok: false, error: '模块文档读不出来' }
  const { body } = parseFrontMatter(text)
  const progress = getSection(body, MODULE_SECTION_HEADINGS.progress)
  const match = progress === null ? null : /完成度\s*[:：]\s*(\d{1,3})/.exec(progress)
  return {
    ok: true,
    name: slug,
    exists: true,
    file,
    score: moduleScoreOf(text),
    declaredProgress: match === null ? null : clampPercent(Number(match[1])),
