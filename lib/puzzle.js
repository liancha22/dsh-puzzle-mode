...
export function auditOf(state) {
  ...
  if (safe.outdated === true) {
    findings.push(finding('doc_outdated', 'warn', null, 'project',
      '文档格式是 puzzle ' + (safe.version ?? '?') + '，当前是 ' + PUZZLE_VERSION + '：旧格式没有 ## 健康性，五维只能靠推导，分数会偏低。',
...
/** 主文档的规范化：front-matter 补版本与 会话:，六节缺哪个补哪个。 */
function migrateMainDoc(text, projectName, planned) {
  ...
  const declared = fields["计划模块"] ?? ""
  let nextBody = body
  for (const key of SECTION_ORDER) {
    const heading = SECTION_HEADINGS[key]
    if (getSection(nextBody, heading) === null) {
      // 缺的小节补成占位行：形状补全是重建的目的，内容仍由人写。
      nextBody = withSection(nextBody, heading, SECTION_HEADINGS[key] === SECTION_HEADINGS.index ? "- （尚未拆分模块）" : "- （待补）")
...
function migrateModuleDoc(text, name, projectText) {
  ...
  let nextBody = body
  for (const key of ["progress", "points", "related", "detail"]) {
    const heading = MODULE_SECTION_HEADINGS[key]
    if (getSection(nextBody, heading) !== null) continue
    nextBody = withSection(nextBody, heading, key === "progress" ? "完成度: 0" : "- （待补）")
    changes.push("补小节 " + heading)
  }

  if (getSection(nextBody, HEALTH_HEADING) === null) {
    const progressText = getSection(nextBody, MODULE_SECTION_HEADINGS.progress) ?? ""
    ...
    if (progress !== null && progress > 0) {
      ...
      if (filled > 0) {
        lines.push("- （v1 → v2 迁移：旧「完成度 " + progress + "」×" + PROGRESS_TO_HEALTH + " = " + seeded + " 分，填给了 " + filled + " 个还拿不到证据的维度；这是起点不是评估，请按真实情况修正）")
      }
    }
    // 位置按当前模板来：`## 健康性` 紧跟 `# 标题`（新版模板在首位），
    // 而不是像 `withSection` 那样插到最后一个"已知六节"之后——健康性不在那六节里。
    nextBody = insertHealthAfterTitle(nextBody, lines.join("\n"))
...
/**
 * 把 `## 健康性` 插到 `# 标题` 之后（当前模板的位置）。
 *
 * 为什么不用 `withSection`：它在「找不到小节」时会插到最后一个**已知小节**之后，
 * 而 `## 健康性` 不在主文档那六节里 —— 结果被插到文件末尾，前面还多一个空行。
 * 实测（拿 v0.1.x 的真实备份跑）就是这么错的，所以这里单独处理标题后的插入。
 */
function insertHealthAfterTitle(body, content) {
  const lines = String(body ?? "").split(/\r?\n/)
  const block = [HEALTH_HEADING, ...String(content ?? "").split("\n")]
  const at = lines.findIndex((line) => /^#\s/.test(line))
  if (at < 0) return [HEALTH_HEADING, ...String(content ?? "").split("\n"), "", ...lines].join("\n")
  // 标题后原有的空行不要保留两份：先吃掉紧邻的空行，再统一插一个空行。
  let next = at + 1
  while (next < lines.length && lines[next].trim() === "") next += 1
  return [...lines.slice(0, at + 1), "", ...block, "", ...lines.slice(next)].join("\n")
}
...
export function createProject(projectRoot, projectName, goal, modules, mode = DEFAULT_MODE, sessionId = '') {
  ...
  try {
    ...
    const bound = bindTo.length > 0 ? bindSession(projectRoot, slug, bindTo[0]) : null
    return {
      ok: true,
      project: slug,
      puzzleDir,
