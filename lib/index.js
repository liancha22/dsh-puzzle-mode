...
const OPS = ['list', 'read', 'show', 'init', 'bind', 'main', 'module', 'health', 'audit', 'mode']
...
function policyText() {
  return [
    ...
    `- 已有项目：\`op:'bind'\` 并给 \`project\` 把本会话绑过去。面板里也有建项目与绑定入口。`,
...
const TOOL_DESCRIPTION = [
  ...
  'op=list 列出现有项目（多项目时先看这个）；op=read 读状态（不建文件）；op=show 读某个模块文档的详情；op=init 一次创建主文档与每个模块一份文档；op=bind 把本会话绑到已有项目；op=main 更新主文档六节之一；op=module 更新（必要时创建）模块文档；op=health 写五维健康性；op=mode 切换执行模式。',
  ...
  ctx.tools.register(defineTool({
    ...
    parameters: {
      op: {
        type: 'string',
        required: true,
        description: 'list 列项目 / read 读状态 / show 读模块详情 / init 新建项目（并绑定本会话）/ bind 绑定已有项目 / main 主文档小节 / module 模块文档小节 / health 写五维健康性 / audit 按五维审查 / mode 切换模式',
        enum: OPS,
      },
    ...
    async execute(args, exec) {
      ...
      if (project === '' && !['read', 'audit', 'init', 'bind'].includes(args.op)) {
