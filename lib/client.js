...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** 图块可点：主文档节 + 模块都去读详情；初始化块没有详情，保持静态。 */
    ...
    function detailBlock(view) {
      ...
      return h(
        ...
        h('h4', null, '模块 · ' + detail.name + (detail.score === undefined ? '' : ' · ' + detail.score + '%')),
        parts,
      )
    }
    ...
    function panelBody(view) {
      ...
      var header = h(
        ...
        data && data.initialized === true ? h('span', { className: 'dshpz-muted' }, data.overall + '%') : null,
        h(
          'button',
          {
      ...
      return h(
          ...
          projectList.length > 1
            ? h(
              ...
              projectList.map(function (item) {
                return h('option', { key: item.name, value: item.name }, item.name + ' · ' + item.overall + '%')
              }),
            )
            : null,
          h(
        ...
        h('div', { className: 'dshpz-grid' }, pieces.map(function (piece) { return tile(piece, view) })),
        detailBlock(view),
        h(
          'div',
    ...
    module.exports = { name: 'dsh-puzzle-mode', apply: apply, questionTemplate: questionTemplate }
